...
function SortableItemRow({ item, idx, lastAddedId, itemsLength, onEdit, onDelete, fmtPL }: {
  ...
  return (
    <tr ref={setNodeRef} style={lastAddedId && idx === itemsLength - 1 ? { ...style, animation: 'rowFlash 1.5s ease-out' } : style}
      ...
      <td className="px-2 py-0.5 text-gray-800 tabular-nums">{item.length_mm || '—'}</td>
      <td className="px-2 py-0.5 text-gray-800 tabular-nums">{item.width_mm || '—'}</td>
      <td className="px-2 py-0.5 text-gray-800 tabular-nums">{item.quantity}</td>
      ...
      <td className="px-3 py-0.5 text-right text-gray-600 tabular-nums">{item.length_mm && item.width_mm ? fmtPL(Number(item.m2), 3) : '—'}</td>
      <td className="px-3 py-0.5 text-right text-gray-600 tabular-nums">{fmtPL(Number(item.price_per_m2), 0)}{item.length_mm === 0 && item.width_mm === 0 ? '/szt' : ''}</td>
      <td className="px-3 py-0.5 text-right text-gray-800 font-medium tabular-nums">{fmtPL(Number(item.total_price), 2)}</td>
...
export default function OrderDetailPage() {
  ...
  const [newWplyka, setNewWplyka] = useState(false)
  const [newNotes, setNewNotes] = useState('')
  const [newCustomPrice, setNewCustomPrice] = useState('')
  const [newCustomQty, setNewCustomQty] = useState('1')
  const [newCustomPasses, setNewCustomPasses] = useState('1')
  ...
  const [newCustomWplyka, setNewCustomWplyka] = useState(false)
  ...
  const handleCustomAdd = useCallback(async () => {
    ...
    const passes = Number(newCustomPasses) || 1
    if (!price) return
    ...
    const err = await addItem({
      ...
      has_wplyka: newCustomWplyka,
      notes: newCustomNotes || null,
      m2: 0,
      price_per_m2: price,
      total_price: Math.round(price * q * passes * 100) / 100,
    ...
    resetCustomForm()
  }, [newCustomPrice, newCustomQty, newCustomVariantId, newCustomHandle, newCustomColorSurcharge, newCustomWplyka, newCustomNotes, variants, addItem, resetCustomForm])
  ...
  const [eiPasses, setEiPasses] = useState('')
  ...
  const wplykaVariant = variants.find((v) => v.name.toLowerCase().includes('wpyłka') || v.name.toLowerCase().includes('wpłyka') || v.name.toLowerCase().includes('wypłka'))
  const wplykaPrice = wplykaVariant ? getPriceForVariant(wplykaVariant.id, variants) : 0
  const totalWplykaCost = items.filter((i) => i.has_wplyka).reduce((s, i) => s + wplykaPrice * i.quantity, 0)
  const totalValue = items.reduce((s, i) => s + Number(i.total_price), 0) + totalHandleCost + totalColorSurchargeCost + totalWplykaCost
  ...
  const startEditItem = (item: typeof items[0]) => {
    ...
    setEiPasses(String(item.passes ?? 1))
  ...
  const saveEditItem = async () => {
    ...
    if (isCustom) {
      await updateItem(editingItemId, {
        ...
        total_price: Math.round(pricePerUnit * q * passes * 100) / 100,
      })
    } else {
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-3 py-1 text-right text-[10px] font-medium text-gray-500 min-w-[104px]">Wartość&nbsp;netto</th>
            ...
            <tbody>
                ...
                if (isEditing) {
                  ...
                  return (
                    <tr key={item.id} ref={editItemRowRef} onBlur={handleRowBlur(editItemRowRef, saveEditItem)} className="border-b border-gray-100 bg-blue-50/30">
                      ...
                      <td className="px-2 py-1 text-right text-gray-600 tabular-nums">{totalEdit > 0 ? fmtPL(totalEdit, 2) : ''}</td>
                      ...
                      <td className="px-2 py-1 text-center"><input type="checkbox" checked={eiWplyka} onChange={(e) => setEiWplyka(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-violet-500 focus:ring-violet-500/30" /></td>
              ...
              {showInlineAdd && (() => {
                ...
                return (
                  <tr ref={itemRowRef} onBlur={handleRowBlur(itemRowRef, handleInlineAdd, () => { if (!Number(newLength) || !Number(newWidth)) setShowInlineAdd(false) })} className="border-b border-gray-100 bg-amber-50/30">
                    ...
                    <td className="px-2 py-1 text-right text-gray-600 tabular-nums">{totalPreview > 0 ? fmtPL(totalPreview, 2) : ''}</td>
                    ...
                    <td className="px-2 py-1 text-center">
                      <input type="checkbox" checked={newWplyka} onChange={(e) => setNewWplyka(e.target.checked)}
                        className="h-4 w-4 rounded border-gray-300 text-violet-500 focus:ring-violet-500/30" />
                    </td>
                    <td className="px-2 py-1">
              ...
              {showCustomAdd && (() => {
                ...
                return (
                  <tr ref={customRowRef} onBlur={handleRowBlur(customRowRef, handleCustomAdd, () => { if (!Number(newCustomPrice)) setShowCustomAdd(false) })} className="border-b border-gray-100 bg-blue-50/30">
                    ...
                    <td className="px-2 py-1 text-right text-gray-600 tabular-nums">{Number(newCustomPrice) > 0 ? fmtPL(Number(newCustomPrice) * (Number(newCustomQty) || 1) * (Number(newCustomPasses) || 1), 2) : ''}</td>
                    <td className="px-2 py-1 text-center">
                    ...
                    <td className="px-2 py-1 text-center">
                      <input type="checkbox" checked={newCustomWplyka} onChange={(e) => setNewCustomWplyka(e.target.checked)}
                        className="h-4 w-4 rounded border-gray-300 text-violet-500 focus:ring-violet-500/30" />
                    </td>
                    <td className="px-2 py-1">
              ...
              {!showInlineAdd && !showCustomAdd && items.length === 0 && (
                <tr><td colSpan={14} className="px-2 py-6 text-center text-xs text-gray-400">
