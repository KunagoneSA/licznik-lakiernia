  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-slate-100">Raport pracownikow</h1>
        <button onClick={exportCsv} className="flex items-center gap-2 rounded-lg bg-slate-800 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {Array.from(summary.entries()).map(([name, s]) => (
          <div key={name} className="rounded-lg bg-slate-800 p-3">
            <p className="text-sm font-medium text-slate-200">{name}</p>
            <p className="text-xs text-slate-400">{s.hours.toFixed(1)}h &middot; {s.cost.toFixed(0)} zl{s.m2 > 0 ? ` · ${s.m2.toFixed(2)} m2` : ''}</p>
          </div>
        ))}
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-800/50">
                ...
                <th className="px-4 py-2 text-left text-xs font-medium text-slate-400">Pracownik</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-slate-400">Operacja</th>
                <th className="px-4 py-2 text-right text-xs font-medium text-slate-400">Godziny</th>
                <th className="px-4 py-2 text-right text-xs font-medium text-slate-400">Stawka</th>
                <th className="px-4 py-2 text-right text-xs font-medium text-slate-400">Koszt</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((l) => (
                <tr key={l.id} className="border-b border-slate-800/50">
                  <td className="px-4 py-2 text-slate-400">{new Date(l.date).toLocaleDateString('pl-PL')}</td>
                  <td className="px-4 py-2 text-slate-200">{l.worker_name}</td>
                  <td className="px-4 py-2 text-slate-300">{l.operation}</td>
                  <td className="px-4 py-2 text-right text-slate-300">{l.hours}</td>
                  <td className="px-4 py-2 text-right text-slate-400">{l.hourly_rate} zl</td>
                  <td className="px-4 py-2 text-right font-medium text-amber-400">{Number(l.cost).toFixed(2)} zl</td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">Brak wpisow</td></tr>
              )}
            </tbody>
          </table>
        </div>
