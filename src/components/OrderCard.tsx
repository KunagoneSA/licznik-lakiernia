...
function getUrgencyClass(plannedDate: string | null, status: string): string {
  ...
  if (!plannedDate) return 'border-l-slate-600'
...
export default function OrderCard({ order }: { order: Order }) {
  return (
    <Link
      ...
      className={`block rounded-lg border-l-4 bg-slate-800 p-3 transition-colors hover:bg-slate-700 ${getUrgencyClass(order.planned_date, order.status)}`}
    >
