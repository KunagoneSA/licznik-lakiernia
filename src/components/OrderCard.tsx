...
export default function OrderCard({ order }: { order: Order }) {
  ...
  return (
    <Link
      ...
      className={`block rounded-lg border-l-4 bg-white shadow-sm p-3 transition-colors hover:bg-gray-50 ${getUrgencyClass(order.planned_date, order.status)}`}
    >
      <div className="flex items-center justify-between">
        <span className="text-sm font-bold text-amber-600">#{order.number}</span>
        {order.planned_date && (
      ...
      <p className="mt-1 text-sm text-gray-800 line-clamp-1">{order.description || 'Brak opisu'}</p>
